...
const journeyMockResponse = (isOk: boolean, status: number, statusText: string, legs?: journeyLeg[]) => {
    const res: any = {
        ok: isOk,
        status: status,
...
describe("JourneyListModel.fetchJourneys()", () => {
    ...
    test("should log error when invalid stationIds are provided", async () => {
        // assertion
        expect(() => new JourneyListModel("invalid-station-id", "invalid-station-id", false)).toThrow(Error);
    });
    ...
    test("should update journeys with valid data when the 200 response is provided", async () => {
        ...
        global.fetch = jest.fn(() => journeyMockResponse(
            true,
            200,
            "OK",
            [dummySuccessfulLeg]
    ...
    test("should log error when the 500 response is provided", async () => {
        ...
        await model.fetchJourneys();
        // assertion
        expect(consoleErrorSpy).toHaveBeenCalledWith(new Error("500 Internal Server Error"));
    });

    test("should fetch correctly when arrival search", async () => {
        // Initiate model with Göttingen to Hannover, arrival search
        model = new JourneyListModel("8000128", "8000152", true);
        // mock the fetch function
        global.fetch = jest.fn(() => journeyMockResponse(
            true,
            200,
